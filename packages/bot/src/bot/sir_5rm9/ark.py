    ...
    async def on_message(message: Message):
        ...
        try:
            ...
            await message.channel.send(
                f"https://magurouhiru.github.io/ASB-web/#/ASB-web/calc?n={resp.n}&h={resp.status.h}&s={resp.status.s}&o={resp.status.o}&f={resp.status.f}&w={resp.status.w}&m={resp.status.m}&t={resp.status.t}{f'&i={resp.status.i}' if resp.status.i is not None else ''}&type={resp.status.type}{f'&level={resp.status.level}' if resp.status.level is not None else ''}",
                silent=True,
                mention_author=True,
            )
